---
name: yadm-utilities
description: This skill should be used when the user asks to "track files with yadm", "check yadm status", "commit to yadm", "sync dotfiles", "add to yadm", "manage OpenCode configuration with yadm", "what should I track in yadm", "use yadm alternates", "create OS-specific dotfiles", "configure yadm for multiple machines", "bootstrap yadm", "run yadm bootstrap", "setup new machine with yadm", or mentions yadm operations for dotfiles management.
version: 0.1.0
metadata:
  author: adamancini
...
This skill provides guidance for managing dotfiles using YADM (Yet Another Dotfiles Manager), with specialized knowledge for tracking OpenCode configuration files, using YADM's alternates feature for multi-system configurations, and bootstrapping new machines.
...
yadm add ~/.config/opencode/agents/new-agent.md

# Track multiple files at once
yadm add ~/.config/opencode/settings.json ~/.config/opencode/CLAUDE.md
...
yadm commit -m "Update OpenCode plugin configuration"
```
...
# Install plugins
opencode plugin install superpowers@superpowers-marketplace
opencode plugin install elements-of-style@superpowers-marketplace

# Clone custom plugin repos
if [ ! -d "$HOME/.config/opencode/plugins/repos/devops-toolkit" ]; then
    git clone git@github.com:username/devops-toolkit.git \
        "$HOME/.config/opencode/plugins/repos/devops-toolkit"
fi
```
...
└── bootstrap.d/
    ...
    ├── 30-claude-code.sh       # OpenCode setup
...
```bash
# macOS work laptop
.config/opencode/opencode.json##os.Darwin,class.Work

# Linux personal desktop
.config/opencode/opencode.json##os.Linux,class.Personal
```
...
- Custom skills (`~/.config/opencode/skills/*/SKILL.md` and supporting files)
- Hookify rules (`~/.config/opencode/hookify.*.local.md`)
- Custom hooks (`~/.config/opencode/hooks/*.py`, `~/.config/opencode/hooks/*.sh`)
- Settings (`~/.config/opencode/settings.json`, `~/.config/opencode/CLAUDE.md`)
- Plugin configuration (`~/.config/opencode/plugins/config.json`, `~/.config/opencode/plugins/installed_plugins.json`, `~/.config/opencode/plugins/known_marketplaces.json`)
- Custom scripts (`~/.config/opencode/*.sh`)
- Bootstrap script (`~/.config/yadm/bootstrap`)

**Never track:**
- Cache directories (`~/.config/opencode/plugins/cache/`, `~/.config/opencode/plugins/marketplaces/`)
- Session state (`~/.config/opencode/file-history/`, `~/.config/opencode/todos/`, `~/.config/opencode/projects/`)
- Debug logs (`~/.config/opencode/debug/`)
- Lock files (`~/.config/opencode/ide/*.lock`)
- History (`~/.config/opencode/history.jsonl`)

**Plugin repositories are special:**
- Separate git repos in `~/.config/opencode/plugins/repos/`
...
5. **Verify setup:**
   ```bash
   ls -la ~/.config/opencode/settings.json  # Check alternates
   opencode plugin list              # Check plugins
...
Update bootstrap to install OpenCode plugins
...
Keep `~/.config/opencode/README.md` synchronized:
...
This skill enables efficient dotfiles management with YADM, including bootstrap automation for new machine setup, alternates for multi-system configurations, and specialized OpenCode configuration tracking.
